 ...
 */
final public class GraftedRootedTree implements RootedTree {

    /**
     * Make a copy of the given rooted tree
     * @param baseTree a rooted tree
     */
    public GraftedRootedTree(RootedTree baseTree, RootedTree leafTree, Taxon target) {
        createNodes(baseTree, leafTree, target, baseTree.getRootNode());
    }
     ...
     * @return
     */
    private Node createNodes(RootedTree baseTree, RootedTree leafTree, Taxon target, Node node) {

        Node newNode;        
        if (baseTree.isExternal(node)) {
        	if (baseTree.getTaxon(node).equals(target)) {
            	newNode = createNodes(leafTree, leafTree.getRootNode());
            }
        	else {
            newNode = createExternalNode(baseTree.getTaxon(node));
        	}

        } else {
            List<Node> children = new ArrayList<Node>();
            for (Node child : baseTree.getChildren(node)) {
                children.add(createNodes(baseTree, leafTree, target, child));
            }
...
//        setHeight(newNode, tree.getHeight(node));
    ...
    private Node createNodes(RootedTree leafTree, Node node) {

        Node newNode;
        if (leafTree.isExternal(node)) {
            newNode = createExternalNode(leafTree.getTaxon(node));

        } else {
            List<Node> children = new ArrayList<Node>();
            for (Node child : leafTree.getChildren(node)) {
                children.add(createNodes(leafTree, child));
            }
            newNode = createInternalNode(children);
        }
