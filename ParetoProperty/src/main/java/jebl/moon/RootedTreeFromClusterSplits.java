 ...
 */
final public class RootedTreeFromClusterSplits implements RootedTree {
	 ...
	 */
	public RootedTreeFromClusterSplits(Map<Set<Taxon>, RootedSplits> clusters, Set<Taxon> root) {
		createNodes(clusters, root);
	}
	 ...
	 * @return
	 */
	private Node createNodes(Map<Set<Taxon>, RootedSplits> clusters, Set<Taxon> node) {

		Node newNode;
		List<Node> children = new ArrayList<Node>();
		if (node.size() > 2) {
			for (Set<Taxon> child : clusters.get(node).getSplits()) {
				children.add(createNodes(clusters, child));
			}
			newNode = createInternalNode(children);

		} else if (node.size() > 1){
			for (Taxon leaf : node) {
				children.add(createExternalNode(leaf));
			}
			newNode = createInternalNode(children);
		} else {
			newNode = createExternalNode(node.iterator().next());
		}        
