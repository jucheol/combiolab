 ...
 */
final public class RootedSubtreeFromNodes implements RootedTree {
     ...
     * @param includedNode
     */
    public RootedSubtreeFromNodes(RootedTree tree, Set<Node> includedNode) {
        createNodes(tree, tree.getRootNode(), includedNode);
    }
     ...
     * @param tree
     * @param node
     * @param includedNode may be empty
     * @return
     */
    private RootedSubtreeNode createNodes(RootedTree tree, Node node, Set<Node> includedNode) {

        RootedSubtreeNode newNode;

        if (includedNode.contains(node)) {                                                
            newNode = createExternalNode(tree, node);

        } else {
            List<RootedSubtreeNode> children = new ArrayList<RootedSubtreeNode>();
            for (Node child : tree.getChildren(node)) {
                RootedSubtreeNode newChild = createNodes(tree, child, includedNode);
                if (newChild != null) {
...
//            setHeight(newNode, tree.getHeight(node));
     ...
     */
    private RootedSubtreeNode createExternalNode(RootedTree tree, Node node) {                
    	Taxon taxon = MyUtils.TaxaSetToSinlge(MyUtils.getDescendantTaxa(tree, node));
        RootedSubtreeNode newNode = new RootedSubtreeNode(taxon);
        externalNodes.put(taxon, newNode);
        return newNode;
    }
